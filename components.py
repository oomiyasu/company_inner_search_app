...
#【問題3】
    ...
    with st.sidebar:
        ...
        st.markdown(
            (
                f'<div style="background-color:white;border:1px solid #CCC;padding:8px;">'
                f'【入力例】\n社員の育成方針に関するMTGの議事録'
                '</div>'
            ),
...
#        st.markdoSwn("こんにちは。私は社内文書の情報をもとに回答する生成AIチャットボットです。サイドバーから利用目的を選択し、画面下部のチャット欄からメッセージを送信してください。")
#【問題3】
        # 参照元のありかに応じて、適したアイコン!を取得
        st.success("こんにちは。私は社内文書の情報をもとに回答する生成AIチャットボットです。サイドバーから利用目的を選択し、画面下部のチャット欄からメッセージを送信してください。")
...
def display_conversation_log():
 ...
 #【問題4】
                        if (
                            ...
                            and message["content"]["main_file_path"].lower().endswith(".pdf")
                        ):
                            st.success(
                                f"{message['content']['main_file_path']}（ページ No.{message['content']['main_page_number']}）",
                                icon=icon,
                            )                       
                        else:
#                           st.success(f"{message['content']['main_file_path']}", icon=icon)
#【問題4】
                             st.success(
                                f"{message['content']['main_file_path']}", 
                                icon=icon
                            )
...
#【問題4】
                                if (
                                    ...
                                    and sub_choice["source"].lower().endswith(".pdf")
                                ):
                                    st.info(
                                        f"{sub_choice['source']}（ページ No.{sub_choice['page_number']}）",
                                        icon=icon,
                                    )
                                else:
                                    st.info(f"{sub_choice['source']}", icon=icon)
...
#【問題4】
            # ページ番号を取得（1ページ目を1として計算）
            main_page_number = llm_response["context"][0].metadata["page"] + 1
            # 「メインドキュメントのファイルパス」と「ページ番号」を表示（PDFファイルのみ） 
            if main_file_path.lower().endswith(".pdf"):
                st.success(
                    f"{main_file_path}（ページ No.{main_page_number}）",
                    icon=icon,
                )
            else:
                st.success(f"{main_file_path}", icon=icon)
...
#【問題4】
                ...
                # 「ファイルパス」と「ページ番号」（PDFファイルのみページ番号を追加）
                if file_path.lower().endswith(".pdf"):
                    file_info = f"{file_path}（ページ No.{page_number}）"
                else:
                    file_info = f"{file_path}"
            else:
                # 「ファイルパス」のみ
